...
public class Monster7 : MonoBehaviour {
    ...
    public float maxHP = 2; // 목숨
    public float HP = 2;
    Transform tran;
    Animator anim;
    public GameObject explo, explo2, explo3, explo4;
   ...
   // public bool isUnBeatTime = false;
    ...
    //    }
        /* 이 로직은 플레이어 추적에 관한 로직으로 현 게임에서 아직은 사용하지 않을 예정이라고 함.
        Vector2 dir = target.position - transform.position;

        transform.position += (target.position - transform.position).normalized * movePower * Time.deltaTime;
        if (dir.x >= 0)
        {
        ...
        */
        ...
        if (!player.RB5 && !player.RB6 && !player.RB7 && player.RB8)
            Destroy(GameObject.Find("Explosion4(Clone)"), 0.2f);        // 일정 시간 후 몬스터 삭제        // 일정 시간 후 몬스터 삭제
    ...
    {
        Vector3 v2 = new Vector3(-0.1f, 0.1f, 0f);
        Vector3 v = new Vector3(0.1f, 0.1f, 0f);
