...
public class Monster5 : MonoBehaviour {
    ...
    public float shootDelayTime2 = 1.5f;
    float delayTimer2 = 0f;
    public float startAttackTime = 2.0f;
    public GameObject Monster5Stone1, Monster5Stone2, Monster5Stone3;
   // Transform Monster5FirePos;
	...
	void Update () {
        ...
        if (!player.RB5 && !player.RB6 && !player.RB7 && player.RB8)
            Destroy(GameObject.Find("Explosion5(Clone)"), 0.2f); Heart();
        Destroy(GameObject.Find("Explosion2(Clone)"), 0.2f); Heart();
    ...
    {
        Attack();
        Invoke("RandomAttack", Random.Range(2, 4));
        Invoke("RandomAttack2", Random.Range(2, 5));
        Invoke("RandomAttack3", Random.Range(3, 5));
        // 
    ...
    public void RandomAttack()
    {
        ...
        if(isAttack)
        {
           GameObject temp = Instantiate(Monster5Stone1, new Vector3(randomX, 1.5f, 0f), Quaternion.identity); //랜덤한 위치와, 화면 제일 위에서 Enemy를 하나 생성해줍니다.
    ...
    {      
        ...
        if (isAttack)
        {
            GameObject temp = Instantiate(Monster5Stone2, new Vector3(randomX2, 1.5f, 0f), Quaternion.identity); //랜덤한 위치와, 화면 제일 위에서 Enemy를 하나 생성해줍니다.
            temp.GetComponent<EnemyStone>().player = this.player;
    ...
    public void RandomAttack3()
    {
        ...
        if (isAttack)
        {
            GameObject temp = Instantiate(Monster5Stone3, new Vector3(randomX3, 1.5f, 0f), Quaternion.identity); //랜덤한 위치와, 화면 제일 위에서 Enemy를 하나 생성해줍니다.
            temp.GetComponent<EnemyStone>().player = this.player;
