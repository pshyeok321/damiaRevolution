...
//    }
    ...
    void Update()
    {
        ...
        if (!player.RB5 && player.RB6 && !player.RB7 && !player.RB8)
            //Destroy(GameObject.Find("Explosion2(Clone)"), 0.5f);
        if (!player.RB5 && !player.RB6 && player.RB7 && !player.RB8)
          //  Destroy(GameObject.Find("Explosion3(Clone)"), 0.5f);
        if (!player.RB5 && !player.RB6 && !player.RB7 && player.RB8)
            Destroy(GameObject.Find("Explosion4(Clone)"), 0.5f);        // 일정 시간 후 몬스터 삭제        // 일정 시간 후 몬스터 삭제
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
        ...
        if (col.gameObject.tag.Equals("Player") && HP > 0)
        {
            ...
            StartCoroutine("IsTrig");            player.HP--;
        ...
        else if (col.gameObject.tag.Equals("Bullet2") && HP > 0)
        {
            ...
            if (!player.RB5 && !player.RB6 && player.RB7 && !player.RB8)
                Instantiate(explo3, transform.position, Quaternion.identity);
            if (!player.RB5 && !player.RB6 && !player.RB7 && player.RB8)
                Instantiate(explo4, transform.position, Quaternion.identity);          //  Hearted.start();
            player.FeverGauge += 1;
        }
        else if (col.gameObject.tag.Equals("Bullet2") && HP <= 0)
        {
    ...
    {
        Vector3 v = new Vector3(0.1f, 0.3f, 0f);
        Vector3 v2 = new Vector3(-0.1f, 0.3f, 0f);
        //int maxItems = 10;
